...
export default function UnitManagementScreen() {
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.centeredMessage}>
          ...
          <MaterialIcons name="arrow-back" size={24} color={Colors.primary} onPress={() => router.back()} style={styles.backButton} />
        </View>
      </View>
    );
  }
  ...
  const [editBuyingUnit, setEditBuyingUnit] = React.useState<'Korokoro' | 'kg' | 'g' | 'sack' | 'piece'>(item.buyingUnit as 'Korokoro' | 'kg' | 'g' | 'sack' | 'piece');
      ...
      <ScrollView style={styles.scrollView}>
        ...
        {!showEdit ? (
          ...
          <View style={styles.editCard}>
            ...
            {/* Selling Unit */}
            <Text style={[styles.editSubtitle, { marginTop: 12 }]}>Selling Unit</Text>
            <View style={styles.editChipRow}>
            ...
            <View style={{ marginTop: 12 }}>
              ...
              {editConversionRateError && (
                <Text style={{ color: '#dc2626', fontSize: 12, marginTop: 4 }}>{editConversionRateError}</Text>
              )}
              <Text style={styles.editHint}>
                1 {editBuyingUnit} = how many {editSellingUnit}?
              </Text>
            </View>
            ...
            <View style={styles.editButtonRow}>
              ...
              <Pressable
                onPress={() => {
                  ...
                  const isValidConversionRate = !isNaN(conversionRateNum) && conversionRateNum > 0;
                  const newThreshold = parseFloat(editLowStockThreshold);
                  const isValidThreshold = !isNaN(newThreshold) && newThreshold > 0;

                  setEditConversionRateError(isValidConversionRate ? null : 'Must be a positive number');
                  setEditLowStockThresholdError(isValidThreshold ? null : 'Must be a positive number');
...
const styles = StyleSheet.create({
  ...
  editChipTextSelected: { fontSize: 14, color: Colors.primary, fontWeight: '700' },
  editChipTextUnselected: { fontSize: 14, color: Colors.onSurfaceVariant, fontWeight: '400' },
  editInput: {
    borderWidth: 1.5,
    borderColor: '#e5e7eb',
    borderRadius: 10,
  ...
  editHint: { fontSize: 11, color: Colors.onSurfaceVariant, marginTop: 4 },
  editButtonRow: { flexDirection: 'row', marginTop: 16, justifyContent: 'space-between' },
  editCancelButton: { flex: 1, backgroundColor: '#d1d5db', borderRadius: 12, paddingVertical: 14, marginRight: 8 },
  editSaveButton: { flex: 1, backgroundColor: Colors.primary, borderRadius: 12, paddingVertical: 14, marginLeft: 8 },
  editCancelButtonText: { color: '#9ca3af', fontSize: 16, fontWeight: '600', textAlign: 'center' },
  editSaveButtonText: { fontSize: 16, fontWeight: '600', textAlign: 'center', color: Colors.onPrimary },
});