    ...
    <Modal animationType="none" transparent visible={visible} statusBarTranslucent>
      ...
      >
        ...
        >
            ...
            {existingItem && (
              <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 6, backgroundColor: '#f0fdf4', borderRadius: 8, padding: 8 }}>
                ...
                <Text style={{ fontSize: 12, color: '#16a34a', marginLeft: 6 }}>
                  "{existingItem.name}" already exists — this will add to its current stock of {existingItem.currentStock} {existingItem.sellingUnit}
                </Text>
              </View>
            )}
          </View>
          ...
          {/* Amount Being Added */}
          <View style={{ marginTop: 16 }}>
            <Text style={{ fontSize: 13, fontWeight: '600', color: Colors.onSurfaceVariant, marginBottom: 6 }}>Amount Being Added</Text>
            <TextInput
              placeholder="e.g. 4"
              placeholderTextColor="#9ca3af"
          ...
          <View style={{ marginTop: 16 }}>
            ...
            <Text style={{ fontSize: 11, color: Colors.onSurfaceVariant, marginBottom: 8 }}>
              How many Korokoro do you get from 1 {buyingUnit}?{'\n'}
              (e.g. 1 sack = 36.5 Korokoro, 1 kg = 0.41 Korokoro)
            </Text>
            <TextInput
          ...
          {/* Total Buying Cost */}
          <View style={{ marginTop: 16 }}>
            <Text style={{ fontSize: 13, fontWeight: '600', color: Colors.onSurfaceVariant, marginBottom: 6 }}>Total Buying Cost (amount paid for the whole delivery)</Text>
            ...
            {buyingPrice && quantity && conversionRate ? (
              <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8 }}>
                <MaterialIcons name="info" size={14} color={Colors.primary} />
                <Text style={{ fontSize: 12, marginLeft: 6, color: Colors.onSurfaceVariant }}>
                  Cost per Korokoro: KES {(
                    parseFloat(buyingPrice) /
                    (parseFloat(quantity) * parseFloat(conversionRate))
                  ).toFixed(2)}
                </Text>
              </View>
            ) : null}
          ...
          <View style={{ marginTop: 16 }}>
            ...
            <Text style={{ fontSize: 11, color: Colors.onSurfaceVariant, marginTop: 4 }}>
              You'll be alerted when stock drops to this level (in Korokoro)
            </Text>
          </View>
        </ScrollView>
