...
const AddStockModal = ({ visible, onClose }: { visible: boolean; onClose: () => void }) => {
  ...
  const isAnyRequiredEmpty =
    !productName.trim() || !quantity.trim() || !price18.trim() ||
    !price14.trim() || !price12.trim() || !price1.trim() || !lowStockThreshold.trim();
  ...
  const resetForm = () => {
    setProductName(''); setDescription(''); setSelectedIcon('grain');
    setSelectedCategory('Cereal'); setQuantity(''); setBuyingUnit('kg');
    setSellingUnit('Korokoro'); setConversionRate(''); setPrice18('');
    setPrice14(''); setPrice12(''); setPrice1(''); setLowStockThreshold('');
    setBuyingPrice(''); setErrors({});
  };
  ...
  return (
    <Modal animationType="none" transparent visible={visible} statusBarTranslucent>
      ...
      <Animated.View style={{
        ...
        <ScrollView style={{ flex: 1 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20, paddingTop: 8, paddingBottom: 24 }}>

          {/* Product Name */}
          <View>
            <Text style={{ fontSize: 13, fontWeight: '600', color: Colors.onSurfaceVariant, marginBottom: 6 }}>Product Name</Text>
            <TextInput
              placeholder="e.g. Maize" placeholderTextColor="#9ca3af"
              ...
              onFocus={() => setProductNameFocused(true)} onBlur={() => setProductNameFocused(false)}
            />
          ...
          <View style={{ marginTop: 16 }}>
            ...
            <TextInput
              placeholder="e.g. Yellow - Dry Shell" placeholderTextColor="#9ca3af"
              ...
              onFocus={() => setDescriptionFocused(true)} onBlur={() => setDescriptionFocused(false)}
            />
          </View>
                ...
                {ICON_OPTIONS.map((iconName) => (
                  <TouchableOpacity key={iconName} onPress={() => setSelectedIcon(iconName)}>
                    <View style={{
                      ...
                      <MaterialIcons name={iconName} size={24} color={selectedIcon === iconName ? Colors.primary : Colors.onSurfaceVariant} />
                    </View>
                  </TouchableOpacity>
                ))}
              </View>
          ...
          <View style={{ marginTop: 16 }}>
            ...
            <TextInput
              placeholder="e.g. 50" placeholderTextColor="#9ca3af" keyboardType="numeric"
          ...
          <View style={{ marginTop: 16 }}>
            ...
            <Text style={{ fontSize: 11, color: Colors.onSurfaceVariant, marginBottom: 8 }}>How many {sellingUnit} do you get from 1 {buyingUnit}?</Text>
            <TextInput
              placeholder="e.g. 36.5" placeholderTextColor="#9ca3af" keyboardType="numeric"
              style={{ borderWidth: 1.5, borderColor: conversionRateFocused ? Colors.primary : '#e5e7eb', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 12, fontSize: 15, color: Colors.onSurface }}
              value={conversionRate} onChangeText={setConversionRate}
              onFocus={() => setConversionRateFocused(true)} onBlur={() => setConversionRateFocused(false)}
            />
            <Text style={{ fontSize: 11, color: Colors.onSurfaceVariant, marginTop: 4 }}>Optional — you can update this later after measuring</Text>
          </View>
          ...
          <View style={{ marginTop: 16 }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <TextInput
                ...
                value={buyingPrice} onChangeText={setBuyingPrice}
                onFocus={() => setBuyingPriceFocused(true)} onBlur={() => setBuyingPriceFocused(false)}
              />
            </View>
            {buyingPrice && price1 ? (
          ...
          <View style={{ marginTop: 16 }}>
            ...
            <TextInput
              placeholder="e.g. 15" placeholderTextColor="#9ca3af" keyboardType="numeric"
              ...
              onFocus={() => setLowStockThresholdFocused(true)} onBlur={() => setLowStockThresholdFocused(false)}
            />
            {errors.lowStockThreshold && <Text style={{ color: '#dc2626', fontSize: 12, marginTop: 4 }}>{errors.lowStockThreshold}</Text>}
            <Text style={{ fontSize: 11, color: Colors.onSurfaceVariant, marginTop: 4 }}>You'll be alerted when stock drops to this level</Text>
          </View>
        </ScrollView>
